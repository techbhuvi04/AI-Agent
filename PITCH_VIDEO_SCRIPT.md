...
A synthetic data generator injects realistic break types (late settlements, refunds, chargebacks, missing orders) at a chosen difficulty and seed, and a custom harness (`eval/harness.py`) measures precision, recall, and auto-clear rate tier by tier, plus wall-clock throughput. The headline run — 5,538 payments across 83 credits, seed 42, medium difficulty — reconciles in 1.72 seconds at roughly 3,212 payments/second, with the ablation table showing exactly what each tier contributes and exactly what the uniqueness gate costs and buys back in precision.

### Stack

Python, pandas, and numpy for the ledger and DP work; Groq (`openai/gpt-oss-20b` by default) for the two LLM touchpoints — T3's break classifier and a natural-language Q&A agent that answers questions from a compact, pre-verified structured summary rather than raw rows, so it can never invent a number; Streamlit and Plotly for a six-page dashboard (Overview, Reconciliation, Analytics, Exception Queue, Cash Position, Ask); pytest for a 115-test suite (114 passing, 1 skipped without an API key).
...
**[0:25–1:10] Approach overview — show the ARCHITECTURE.md tier diagram or a whiteboard-style slide of T0–T4**
...
**[1:10–2:00] Live demo — screen-record the Streamlit dashboard: Overview page, then Reconciliation page**

"Here's the dashboard a controller would actually use. [click through Overview] Auto-clear rate, precision, rupees verified, rupees at risk, throughput — all headline numbers. [click Reconciliation] Every payment's tier-by-tier journey, and a downloadable audit trail — one row per payment, which tier cleared it, what evidence backs the decision, timestamped and tied to a run ID. Nothing here is a black box."

**[2:00–3:00] The key innovation + AI judgment — show the T4 uniqueness-gate table from ARCHITECTURE.md or the Analytics page**

"The part I'm proudest of is in the verification tier. Finding *a* subset of payments that sums to the credit is easy — with 80 to 160 candidates, there are usually several subsets that sum to the same number. Picking one and calling it solved is a coin flip dressed up as arithmetic. So the verifier counts *every* distinct subset that sums to the target. Exactly one solution means the match is mathematically proven. Two to ten means only the payments common to every possible solution get cleared. More than ten, it's rejected outright and goes to a human queue. That one change dropped my auto-clear rate on purpose — and pushed precision from 0.58 to over 0.80. For a financial ledger, an honest exception beats an unevidenced clear, every time."

**[3:00–4:00] Failure recovery — show a before/after, e.g. terminal timing output or the performance table**

"Two things broke along the way, and both changed the design. First: my original subset-sum solver hit an iteration cap and silently gave up on solvable credits — it wasn't just slow, it was wrong. I rewrote the reachable-sum search as a single big-integer bitmask, so a whole DP step became one bitwise operation instead of a Python loop. That took the full pipeline from 233 seconds to 1.7 seconds, and because the search could now actually finish, it cleared 55 credits instead of 30 — a correctness fix disguised as a performance fix. Second: I originally asked the LLM to propose which exact payment IDs made up a match. It was bad at it — worse than the deterministic solver I already had. So I redesigned T3 to have the LLM classify the *type* of break instead, and let the DP keep owning every number."

**[4:00–4:40] Results + rigor — show the ablation/metrics table**

"On a synthetic benchmark of 5,538 payments across 83 credits with injected refunds, chargebacks, and late settlements: the engine reconciles in 1.7 seconds at over 3,000 payments a second, and the full evaluation harness — precision, recall, auto-clear rate, per-tier ablation — is reproducible with one command. A hundred and fourteen of a hundred and fifteen unit tests pass; the one skip only fires without an API key configured."

**[4:40–5:00] Close**

"This is reconciliation you can actually trust a cash forecast on, because every number it reports is one the system can prove, not just compute. Thanks for watching."
...
"I built a five-tier pipeline: key joins, then date arithmetic, then a dynamic-programming subset-sum solver, escalating only what the cheaper tier couldn't clear. An LLM only enters at the very end, and only to classify *why* something failed — never to do the math. Every AI suggestion gets independently re-verified by deterministic code before anything is trusted."

**[0:50–1:35] The innovation + AI judgment**

"The core contribution is the verification gate: finding a subset of payments that sums to a bank credit is easy, but with over a hundred candidates, several different subsets often sum to the same number. So instead of stopping at the first match, I count *every* subset that reaches the target. Exactly one means the match is mathematically proven; more than ten means it's rejected and routed to a human. That deliberately traded auto-clear rate for precision — and took precision from 0.58 to over 0.80. In finance, an honest exception beats a fabricated match."

**[1:35–2:05] Failure recovery**

"Two failures shaped this. My first subset-sum solver hit an iteration cap and silently abandoned solvable credits — I rewrote it as a single bitmask operation, cutting runtime from 233 seconds to 1.7 seconds and, more importantly, letting it actually finish and clear correctly. And my first LLM design asked it to propose the matching payments directly — it was worse at that than my own solver, so I redesigned it to classify break types only, and kept every number deterministic."

**[2:05–2:30] Results + close**

"On 5,538 payments and 83 credits with injected breaks, it reconciles in 1.7 seconds at 3,000-plus payments a second, with 114 of 115 tests passing and a full reproducible evaluation harness. This is reconciliation a finance team can actually build a cash forecast on. Thanks for watching."
